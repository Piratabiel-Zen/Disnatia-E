...
battle = battle.replace(
  ...
  "border: draggingId === token.id ? \`2px solid \${info.ring}\` : isSelected ? '2px solid #fff' : \`2px solid \${healthRingColor}\`,"
);
battle = battle.replace(
  "boxShadow: draggingId === token.id ? \`0 0 14px \${info.color}\` : isSelected ? \`0 0 14px \${info.color}\` : '0 2px 8px rgba(0,0,0,0.4)',",
  "boxShadow: draggingId === token.id ? \`0 0 14px \${info.color}\` : isSelected ? \`0 0 14px \${info.color}\` : (displayMaxHp>0 ? \`0 0 8px \${healthRingColor}55,0 2px 8px rgba(0,0,0,.4)\` : '0 2px 8px rgba(0,0,0,0.4)'),"
);

for (const marker of ['formEnemyId','enemyId:tpl.enemyId','Vida vinculada','tokenVitalState','setTokenLinkedHp','undoTokenHp','healthRingColor','enemyMaxHpForToken']) {
