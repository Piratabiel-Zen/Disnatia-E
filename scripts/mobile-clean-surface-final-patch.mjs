...
for (const file of [appFile, expFile, expCssFile, gameCssFile]) must(fs.existsSync(file), `arquivo ausente: ${path.relative(root, file)}`);
...
if (!gameCss.includes(GAME_CSS_MARKER)) gameCss += `\n\n${GAME_CSS_MARKER}\n@media(max-width:900px){\n  .game3-root:not(.game3-tab-session):not(.game3-mode-combat) .g3-mobile-session-hub,.game3-root:not(.game3-tab-session):not(.game3-mode-combat) .g3-mobile-hud-toggle,.game3-root:not(.game3-tab-session):not(.game3-mode-combat) .g3-actionbar,.game3-root:not(.game3-tab-session):not(.game3-mode-combat) .g3-top-context{display:none!important}\n  .g3-mobile-session-hub{height:42px!important;bottom:68px!important;left:8px!important;right:60px!important;padding:3px!important;border-radius:11px!important;box-shadow:0 8px 20px rgba(0,0,0,.32)!important;opacity:.62!important}.g3-mobile-session-hub.combat{opacity:.9!important}\n  .g3-mobile-session-hub button span{font-size:12px!important}.g3-mobile-session-hub button small{font-size:5.5px!important}\n  .g3-mobile-hud-toggle{width:42px!important;height:42px!important;right:10px!important;bottom:68px!important;border-radius:12px!important;box-shadow:0 8px 20px rgba(0,0,0,.34)!important}.g3-mobile-hud-toggle span{font-size:14px!important}.g3-mobile-hud-toggle small{font-size:5.5px!important}.dice-widget{bottom:68px!important}\n}\n`;
...
fs.writeFileSync(appFile, app); fs.writeFileSync(expFile, exp); fs.writeFileSync(expCssFile, expCss); fs.writeFileSync(gameCssFile, gameCss);
