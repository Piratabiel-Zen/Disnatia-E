...
// Artefatos com conteúdo para I e II
const ARTEFATOS_DATA=[
  {
    id:'artefato-1',
    name:'Cristal Cristalizado da Gota de Água',
    icon:'💎',
    descricao:`O Cristal Cristalizado da Gota de Água é um artefato muito poderoso, expelido do próprio corpo do Homem Água durante um dos momentos em que sua essência foi mais brutalmente drenada.\n\nO usuário que o carrega ganha.... (o livro não descreve)`,
    localizacao:'Desconhecida',
    origem:'Expelido do corpo do Homem Água',
  },
  {
    id:'artefato-2',
    name:'Sandaliers Six',
    icon:'👡',
    descricao:`Quem possui esse artefato pode estar onde bem entender. Espaço e tempo não o param, podendo se movimentar de forma livre em qualquer momento — um teleporte instantâneo sem restrições de distância ou dimensão.`,
    localizacao:'Desconhecida',
    origem:'Desconhecida',
  },
  {id:'artefato-3',name:'Artefato III',icon:'◆',descricao:'',localizacao:'',origem:''},
  {id:'artefato-4',name:'Artefato IV',icon:'◆',descricao:'',localizacao:'',origem:''},
  {id:'artefato-5',name:'Artefato V',icon:'◆',descricao:'',localizacao:'',origem:''},
  {id:'artefato-6',name:'Artefato VI',icon:'◆',descricao:'',localizacao:'',origem:''},
];

// Regras atualizadas com teste de reflexo, crítico de dano máximo e nível 30
const RULES_DATA=[
  ...
  {icon:'🌀',title:'Teste de Reflexo',body:`O Teste de Reflexo é uma reação de emergência que pode ser usada mesmo fora do turno do personagem.\n\nCondição de uso:\n• O personagem precisa ter exatamente 1 de Vigor Cósmico disponível.\n• Pode ser usado no máximo 2 vezes por combate.\n\nComo funciona:\n• Quando um ataque estiver prestes a atingir o personagem, ele pode declarar um Teste de Reflexo gastando 1 Vigor Cósmico.\n• Role 1D20 — se o resultado for 16 ou mais, o personagem esquiva do ataque completamente, sem receber dano.\n• Se o resultado for 15 ou menos, o ataque acerta normalmente.\n\n⚠️ O uso do Teste de Reflexo consome o 1 VC disponível, independente do resultado.\n\nEsta habilidade representa o instinto puro de sobrevivência — a última reserva de energia cósmica sendo usada para garantir a vida.`},
  ...
  {icon:'✦',title:'Progressão & XP',body:`Nível máximo: 30\n\nTítulos por Nível:\n• Nível 1–3 → Aprendiz Cósmico\n• Nível 4–6 → Portador do Destino\n• Nível 7–9 → Arauto do Fim\n• Nível 10–19 → Transcendente\n• Nível 20–29 → Além do Horizonte\n• Nível 30 → Absoluto\n\nDesbloqueio de Especiais:\n• Especial I — desbloqueado ao atingir Nível 3\n• Especial II — desbloqueado ao atingir Nível 7\n\nNovas habilidades:\n• A cada 4 níveis após o Especial II, o personagem desbloqueia uma nova habilidade especial (níveis 11, 15, 19, 23, 27...).\n• Essas habilidades são definidas pelo Mestre de acordo com a narrativa e evolução do personagem.\n\nOs valores de XP por nível são definidos pelo Mestre conforme o ritmo da campanha.`},
];
...
// HP sem limite de máximo. hp_bonus = vida extra por curas/itens
...
function SheetCard({sheet,onChange,onDelete}){
  ...
  const label=v=>v<=3?'Aprendiz Cósmico':v<=6?'Portador do Destino':v<=9?'Arauto do Fim':v<=19?'Transcendente':v<=29?'Além do Horizonte':'Absoluto';
    ...
    <div style={{border:`1px solid ${sheetColor}44`,borderRadius:14,overflow:'hidden',background:'rgba(8,10,22,0.9)',marginBottom:18,boxShadow:`0 4px 20px ${sheetGlow}`}}>
      ...
      <div style={{padding:'16px 18px'}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(138px,1fr))',gap:9,marginBottom:16}}>
          ...
          {/* HP — sem limite máximo + barra de vida bônus */}
          <div style={{background:'rgba(232,25,60,0.07)',border:'1px solid rgba(232,25,60,0.2)',borderRadius:10,padding:'12px 14px',gridColumn:'span 2'}}>
            ...
            <div style={{marginBottom:10}}>
              <div style={{fontSize:10,color:'rgba(255,255,255,0.3)',fontFamily:'Cinzel,serif',marginBottom:5,letterSpacing:'0.15em'}}>VIDA BASE</div>
              <div style={{display:'flex',alignItems:'center',gap:7}}>
                <button onClick={()=>f('hp',Math.max(0,sheet.hp-1))} style={{width:28,height:28,borderRadius:5,border:'1px solid rgba(232,25,60,0.3)',background:'rgba(232,25,60,0.1)',color:'#E8193C',cursor:'pointer',fontSize:16,lineHeight:1,padding:0}}>−</button>
                <input
                  type="number"
                  min={0}
                  value={sheet.hp}
                  onChange={e=>f('hp',Math.max(0,+e.target.value))}
                  style={{width:70,textAlign:'center',fontFamily:'Cinzel,serif',fontSize:22,fontWeight:700,color:'#E8193C',background:'transparent',border:'none',padding:'0 4px'}}
                />
                <button onClick={()=>f('hp',sheet.hp+1)} style={{width:28,height:28,borderRadius:5,border:'1px solid rgba(232,25,60,0.3)',background:'rgba(232,25,60,0.1)',color:'#E8193C',cursor:'pointer',fontSize:16,lineHeight:1,padding:0}}>+</button>
              </div>
              <div style={{marginTop:6,height:4,background:'rgba(255,255,255,0.06)',borderRadius:2}}>
                <div style={{height:'100%',width:'100%',background:'#E8193C',borderRadius:2,opacity:sheet.hp>0?1:0.15}}/>
              </div>
              <div style={{fontSize:10,color:'rgba(255,255,255,0.18)',marginTop:3}}>sem limite máximo</div>
            </div>

            {/* Vida bônus */}
            <div style={{borderTop:'1px solid rgba(232,25,60,0.12)',paddingTop:10}}>
              <div style={{fontSize:10,color:'rgba(80,220,140,0.7)',fontFamily:'Cinzel,serif',marginBottom:5,letterSpacing:'0.15em'}}>VIDA BÔNUS <span style={{fontSize:9,color:'rgba(255,255,255,0.2)',fontStyle:'italic'}}>(curas · itens · status)</span></div>
              <div style={{display:'flex',alignItems:'center',gap:7}}>
                <button onClick={()=>f('hp_bonus',Math.max(0,(sheet.hp_bonus||0)-1))} style={{width:28,height:28,borderRadius:5,border:'1px solid rgba(80,220,140,0.25)',background:'rgba(80,220,140,0.07)',color:'#50DC8C',cursor:'pointer',fontSize:16,lineHeight:1,padding:0}}>−</button>
                <input
                  type="number"
                  min={0}
                  value={sheet.hp_bonus||0}
                  onChange={e=>f('hp_bonus',Math.max(0,+e.target.value))}
                  style={{width:70,textAlign:'center',fontFamily:'Cinzel,serif',fontSize:20,fontWeight:700,color:'#50DC8C',background:'transparent',border:'none',padding:'0 4px'}}
                />
                <button onClick={()=>f('hp_bonus',(sheet.hp_bonus||0)+1)} style={{width:28,height:28,borderRadius:5,border:'1px solid rgba(80,220,140,0.25)',background:'rgba(80,220,140,0.07)',color:'#50DC8C',cursor:'pointer',fontSize:16,lineHeight:1,padding:0}}>+</button>
              </div>
              <div style={{marginTop:6,height:4,background:'rgba(255,255,255,0.06)',borderRadius:2}}>
                <div style={{height:'100%',width:(sheet.hp_bonus||0)>0?'100%':'0%',background:'#50DC8C',borderRadius:2,transition:'width 0.3s',opacity:(sheet.hp_bonus||0)>0?1:0.15}}/>
              </div>
              {(sheet.hp_bonus||0)>0&&<div style={{fontSize:10,color:'rgba(80,220,140,0.6)',marginTop:3}}>vida total: {sheet.hp + (sheet.hp_bonus||0)}</div>}
            </div>
          </div>
...
const ENEMY_HP_MAX=400;

const newEnemySkill=()=>({id:Date.now()+Math.random(),nome:'',descricao:'',dano:'',cost:0,tempo:'',tipo:'normal'});

const newEnemy=id=>({id,nome:'',tipo:'',hp:10,vigos:10,alcance:'',forca:0,agilidade:0,durabilidade:0,inteligencia:0,percepcao:0,foto:'',habilidades:[newEnemySkill()],notas:''});

function EnemySkillEditor({skill,color,onChange,onDelete}){
  const update=(key,value)=>onChange({...skill,[key]:value});
  return(
    <div style={{background:'rgba(255,255,255,0.02)',border:`1px solid ${color}22`,borderRadius:10,padding:'12px 14px',display:'flex',flexDirection:'column',gap:10}}>
      <div style={{display:'flex',gap:10,alignItems:'flex-end',flexWrap:'wrap'}}>
        <div style={{flex:1,minWidth:180}}>
          <label style={{fontSize:10,letterSpacing:'0.25em',color:'#7A4040',fontFamily:'Cinzel,serif',display:'block',marginBottom:5,textTransform:'uppercase'}}>Nome da habilidade</label>
          <input value={skill.nome} onChange={e=>update('nome',e.target.value)} placeholder="Ex: Golpe Sombrio" style={{width:'100%'}}/>
        </div>
        <div style={{width:120}}>
          <label style={{fontSize:10,letterSpacing:'0.25em',color:'#7A4040',fontFamily:'Cinzel,serif',display:'block',marginBottom:5,textTransform:'uppercase'}}>Dano</label>
          <input value={skill.dano} onChange={e=>update('dano',e.target.value)} placeholder="Ex: 1D8 + 2" style={{width:'100%'}}/>
        </div>
        <div style={{width:100}}>
          <label style={{fontSize:10,letterSpacing:'0.25em',color:'#7A4040',fontFamily:'Cinzel,serif',display:'block',marginBottom:5,textTransform:'uppercase'}}>Custo VC</label>
          <input type="number" min={0} value={skill.cost} onChange={e=>update('cost',Number(e.target.value))} style={{width:'100%'}}/>
        </div>
        <div style={{width:130}}>
          <label style={{fontSize:10,letterSpacing:'0.25em',color:'#7A4040',fontFamily:'Cinzel,serif',display:'block',marginBottom:5,textTransform:'uppercase'}}>Tempo de uso</label>
          <input value={skill.tempo} onChange={e=>update('tempo',e.target.value)} placeholder="Ex: 3 rodadas" style={{width:'100%'}}/>
        </div>
        <div style={{width:120}}>
          <label style={{fontSize:10,letterSpacing:'0.25em',color:'#7A4040',fontFamily:'Cinzel,serif',display:'block',marginBottom:5,textTransform:'uppercase'}}>Tipo</label>
          <select value={skill.tipo} onChange={e=>update('tipo',e.target.value)} style={{width:'100%'}}>
            <option value="normal">Normal</option>
            <option value="especial">Especial</option>
            <option value="passiva">Passiva</option>
          </select>
        </div>
        <button onClick={onDelete} style={{background:'rgba(232,25,60,0.1)',border:'1px solid rgba(232,25,60,0.3)',color:'#E8193C',borderRadius:6,cursor:'pointer',padding:'6px 10px',fontSize:12,height:34}}>✕</button>
      </div>
      <div>
        <label style={{fontSize:10,letterSpacing:'0.25em',color:'#7A4040',fontFamily:'Cinzel,serif',display:'block',marginBottom:5,textTransform:'uppercase'}}>Descrição</label>
        <textarea value={skill.descricao} onChange={e=>update('descricao',e.target.value)} placeholder="Descreva o efeito, alcance, condição ou comportamento desta habilidade..." rows={3} style={{width:'100%',resize:'vertical',lineHeight:1.7}}/>
      </div>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',gap:12,flexWrap:'wrap',borderTop:'1px solid rgba(255,255,255,0.05)',paddingTop:8}}>
        <span style={{fontSize:10,color:color,fontFamily:'Cinzel,serif',letterSpacing:'0.12em'}}>{skill.tipo?.toUpperCase()||'HABILIDADE'}</span>
        <div style={{display:'flex',gap:12,alignItems:'center',flexWrap:'wrap'}}>
          <span style={{fontSize:11,color:`${color}CC`,fontFamily:'Cinzel,serif'}}>⚔ {skill.dano||'sem dano'}</span>
          <span style={{fontSize:11,color:`${color}CC`,fontFamily:'Cinzel,serif'}}>{skill.cost} VC</span>
          <span style={{fontSize:10,color:'rgba(255,255,255,0.18)'}}>⏱ {skill.tempo||'sem tempo definido'}</span>
        </div>
      </div>
    </div>
  );
      ...
      <div style={{padding:'16px 18px'}}>
          ...
          <div style={{background:'rgba(232,25,60,0.09)',border:'1px solid rgba(232,25,60,0.25)',borderRadius:10,padding:'12px 14px'}}>
            ...
            <div style={{display:'flex',alignItems:'center',gap:7}}>
              ...
              <input
                ...
                max={ENEMY_HP_MAX}
                value={enemy.hp}
                onChange={e=>f('hp',Math.min(ENEMY_HP_MAX,Math.max(0,+e.target.value)))}
                style={{width:64,textAlign:'center',fontFamily:'Cinzel,serif',fontSize:20,fontWeight:700,color:'#E8193C',background:'transparent',border:'none'}}
              />
              <button onClick={()=>f('hp',Math.min(ENEMY_HP_MAX,enemy.hp+1))} style={{width:26,height:26,borderRadius:5,border:'1px solid rgba(232,25,60,0.3)',background:'rgba(232,25,60,0.1)',color:'#E8193C',cursor:'pointer',fontSize:16,lineHeight:1,padding:0}}>+</button>
            </div>
            <div style={{marginTop:7,height:3,background:'rgba(255,255,255,0.06)',borderRadius:2}}><div style={{height:'100%',width:`${Math.min(100,(enemy.hp/ENEMY_HP_MAX)*100)}%`,background:'#E8193C',borderRadius:2,transition:'width 0.3s'}}/></div>
            <div style={{fontSize:10,color:'rgba(255,255,255,0.18)',marginTop:3}}>máx {ENEMY_HP_MAX}</div>
          </div>
        ...
        <div style={{marginBottom:14}}>
          <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',gap:12,marginBottom:8,flexWrap:'wrap'}}>
            ...
            <button onClick={()=>f('habilidades',[...enemySkills,newEnemySkill()])} style={{padding:'6px 12px',borderRadius:7,border:`1px solid ${ENEMY_COLOR}33`,background:`${ENEMY_COLOR}10`,color:ENEMY_COLOR,cursor:'pointer',fontFamily:'Cinzel,serif',fontSize:11,letterSpacing:'0.06em'}}>+ Adicionar habilidade</button>
          </div>
          <div style={{display:'flex',flexDirection:'column',gap:8}}>
            {enemySkills.length===0&&(<div style={{padding:14,borderRadius:10,border:'1px dashed rgba(255,255,255,0.08)',color:'#6A4A4A',textAlign:'center',fontFamily:'Cinzel,serif',fontSize:11,letterSpacing:'0.06em'}}>Nenhuma habilidade cadastrada.</div>)}
            {enemySkills.map((skill,index)=>(<EnemySkillEditor key={skill.id||index} skill={skill} color={ENEMY_COLOR} onChange={updatedSkill=>{const updated=[...enemySkills];updated[index]=updatedSkill;f('habilidades',updated);}} onDelete={()=>{const updated=enemySkills.filter((_,i)=>i!==index);f('habilidades',updated);}}/>))}
          </div>
        </div>
      ...
      {page===2&&(
        <div style={{animation:'fadeIn 0.4s ease'}}>
          ...
          <div style={{display:'flex',flexDirection:'column',gap:12}}>
            {ARTEFATOS_DATA.map((art,i)=>{const revealed=artefatosUnlocked[art.id]||false;const hasContent=!!(art.descricao);return(<div key={art.id} style={{border:`1px solid ${revealed?'rgba(232,160,32,0.3)':'rgba(255,255,255,0.05)'}`,borderRadius:11,background:revealed?'rgba(232,160,32,0.04)':'rgba(255,255,255,0.014)',overflow:'hidden'}}><div style={{padding:'12px 16px',display:'flex',alignItems:'center',gap:10}}><span style={{fontSize:18,opacity:revealed?1:0.3}}>{art.icon}</span><div style={{flex:1}}><div style={{fontFamily:'Cinzel,serif',fontSize:13,color:revealed?'#E8D8C0':'#4A4050',fontWeight:600}}>{revealed?art.name:`Artefato ${i+1} — Selado`}</div><div style={{fontSize:10,color:revealed?'rgba(232,160,32,0.6)':'#3A3040',letterSpacing:'0.18em',fontFamily:'Cinzel,serif',marginTop:2}}>{revealed?'ARTEFATO REVELADO':'SELADO POR MAGIA PODEROSA'}</div></div><button onClick={()=>toggleArtefato(art.id)} style={{padding:'5px 12px',borderRadius:5,border:`1px solid ${revealed?'rgba(232,160,32,0.35)':'rgba(232,25,60,0.25)'}`,background:revealed?'rgba(232,160,32,0.07)':'rgba(232,25,60,0.05)',color:revealed?'#E8A020':'#6A4A4A',cursor:'pointer',fontFamily:'Cinzel,serif',fontSize:10,letterSpacing:'0.08em'}}>{revealed?'🔒 Selar':'🔓 Revelar'}</button></div>
            {!revealed&&(<div style={{padding:'16px',textAlign:'center',borderTop:'1px solid rgba(255,255,255,0.04)'}}><div style={{fontSize:22,marginBottom:6,opacity:0.2}}>◆</div><div style={{fontSize:12,color:'#3A3040',fontFamily:'Cinzel,serif',letterSpacing:'0.08em',fontStyle:'italic'}}>Este artefato permanece oculto por uma magia poderosa.</div><div style={{fontSize:11,color:'#2A2030',marginTop:4}}>Seu poder será revelado conforme a história avança.</div></div>)}
            {revealed&&(<div style={{padding:'16px 18px',borderTop:'1px solid rgba(232,160,32,0.1)',display:'flex',flexDirection:'column',gap:12}}>
              {hasContent?(
                <>
                  <div>
                    <div style={{fontSize:10,letterSpacing:'0.3em',color:'#E8A020',fontFamily:'Cinzel,serif',marginBottom:8,textTransform:'uppercase'}}>Descrição</div>
                    <div style={{fontSize:14,color:'#9A8A7A',lineHeight:1.85,fontStyle:'italic',whiteSpace:'pre-line'}}>{art.descricao}</div>
                  </div>
                  {art.localizacao&&(<div style={{display:'flex',gap:24,flexWrap:'wrap'}}>
                    <div><div style={{fontSize:10,color:'#5A5070',fontFamily:'Cinzel,serif',letterSpacing:'0.2em',marginBottom:4,textTransform:'uppercase'}}>Localização</div><div style={{fontSize:13,color:'#8A7A6A'}}>{art.localizacao}</div></div>
                    {art.origem&&<div><div style={{fontSize:10,color:'#5A5070',fontFamily:'Cinzel,serif',letterSpacing:'0.2em',marginBottom:4,textTransform:'uppercase'}}>Origem</div><div style={{fontSize:13,color:'#8A7A6A'}}>{art.origem}</div></div>}
                  </div>)}
                </>
              ):(
