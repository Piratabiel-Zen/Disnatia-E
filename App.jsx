...
// ─── 🌦️ ATMOSPHERE WIDGET — top-left, icon only ───────────────────────────────
function AtmosphereWidget({ masterMode, atmosphere, onSet }) {
  ...
  return (
    <div style={{ position: 'fixed', top: 14, left: 16, zIndex: 200 }}>
      {!open && (
        <button
          ...
          style={{
            ...
            border: `1px solid ${atm.accent}55`,
            background: `${atm.accent}18`,
      ...
      {open && masterMode && (
        <div style={{
          background: 'rgba(10,12,28,0.97)', border: '1px solid rgba(168,85,247,0.3)',
          borderRadius: 14, padding: 14, width: 200,
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
            {Object.entries(ATMOSPHERES).map(([key, a]) => (
              <button key={key} onClick={() => { onSet(key); setOpen(false); pushToast(`Atmosfera: ${a.label}`, a.icon, a.accent); }} style={{
                ...
                border: `1px solid ${atmosphere === key ? a.accent + '66' : 'rgba(255,255,255,0.06)'}`,
      ...
      <div style={{padding:'16px 18px'}}>
        <div style={{display:'flex', gap:10, alignItems:'flex-end', marginBottom:16, flexWrap:'wrap'}}>
          <div style={{flex:1, minWidth:130}}>
            ...
            {masterMode ? <input value={item.nome} onChange={e => f('nome', e.target.value)} placeholder="Ex: Besta das Sombras..." style={{width:'100%'}}/> : <div style={{fontSize:15, color:'#E8D8C0', padding:'4px 0', fontFamily:'Cinzel,serif', fontWeight:600}}>{item.nome || 'Desconhecida'}</div>}
          </div>
          <div style={{width:140}}>
            ...
            {masterMode ? <select value={item.nivelAmeaca} onChange={e => f('nivelAmeaca', e.target.value)} style={{width:'100%', color:corBase, fontWeight:'bold'}}><option value="Baixo">Baixo</option><option value="Médio">Médio</option><option value="Alto">Alto</option><option value="Supremo">Supremo</option><option value="Catastrófico">Catastrófico</option></select> : <div style={{fontSize:14, color:corBase, padding:'4px 0', fontWeight:'bold', fontFamily:'Cinzel,serif'}}>{item.nivelAmeaca}</div>}
          </div>
        ...
        <div>
          ...
          {masterMode ? <textarea value={item.descricao} onChange={e => f('descricao', e.target.value)} placeholder="Descreva os hábitos, a aparência bizarra, e táticas de combate da criatura..." rows={5} style={{width:'100%', resize:'vertical', lineHeight:1.7}}/> : <div style={{fontSize:14, color:'#9A8A7A', lineHeight:1.85, whiteSpace:'pre-wrap', fontStyle:'italic', background:'rgba(255,255,255,0.02)', padding:'12px', borderRadius:'8px', border:'1px solid rgba(255,255,255,0.05)'}}>{item.descricao || 'Nenhum registro sobre o comportamento desta criatura foi feito até o momento.'}</div>}
        </div>
      </div>
    </div>
  );
...
function BestiarioSection({ masterMode }) {
  ...
  useEffect(() => {
    const unsub = onSnapshot(collection(db, 'bestiario'), snap => {
      ...
      setBestiario(data); setLoaded(true);
    });
    return () => unsub();
  }, []);
  ...
  return (
    <div style={{maxWidth:760, margin:'0 auto', padding:'40px 24px 80px'}}>
      ...
      {loaded && masterMode && <div style={{display:'flex', justifyContent:'flex-end', marginBottom:18}}><button onClick={add} style={{padding:'8px 20px', borderRadius:8, border:'1px solid rgba(232,160,32,0.4)', background:'rgba(232,160,32,0.1)', color:'#E8D8C0', cursor:'pointer', fontFamily:'Cinzel,serif', fontSize:12, letterSpacing:'0.08em'}}>+ Catalogar Criatura</button></div>}
      {loaded && bestiario.length > 0 && (
        <div style={{ display: 'flex', gap: 12, marginBottom: 24, flexWrap: 'wrap', background: 'rgba(255,255,255,0.02)', padding: '14px', borderRadius: '12px', border: '1px solid rgba(232,160,32,0.15)' }}>
          <div style={{ flex: 1, minWidth: 200 }}><label style={{fontSize:9, letterSpacing:'0.2em', color:'rgba(232,160,32,0.7)', fontFamily:'Cinzel,serif', display:'block', marginBottom:6, textTransform:'uppercase'}}>Pesquisar</label><input type="text" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} placeholder="Nome da criatura..." style={{ width: '100%', background: 'rgba(0,0,0,0.4)' }}/></div>
          <div style={{ width: 160, flexShrink: 0 }}><label style={{fontSize:9, letterSpacing:'0.2em', color:'rgba(232,160,32,0.7)', fontFamily:'Cinzel,serif', display:'block', marginBottom:6, textTransform:'uppercase'}}>Categoria</label><select value={filterAmeaca} onChange={e => setFilterAmeaca(e.target.value)} style={{ width: '100%', background: 'rgba(0,0,0,0.4)' }}><option value="Todas">Todas as Ameaças</option><option value="Baixo">Baixo</option><option value="Médio">Médio</option><option value="Alto">Alto</option><option value="Supremo">Supremo</option><option value="Catastrófico">Catastrófico</option></select></div>
        </div>
      )}
      ...
      {loaded && bestiario.length > 0 && filteredBestiario.length === 0 && <div style={{textAlign:'center', padding:30, color:'#7A6A5A', fontFamily:'Cinzel,serif', fontSize:13}}>Nenhuma criatura encontrada com estes filtros.</div>}
      {filteredBestiario.map(item => <BestiarioCard key={item.id} item={item} onChange={d => upd(item.id, d)} onDelete={() => del(item.id)} masterMode={masterMode} />)}
    </div>
  );
}
...
function MasterPagesBook({masterMode}){
  ...
  return(
    <div>
      <div style={{marginBottom:20,padding:'14px 18px',border:'1px solid rgba(168,85,247,0.18)',borderRadius:10,background:'rgba(168,85,247,0.05)',fontFamily:'Crimson Text,Georgia,serif',fontSize:14,color:'#9A8A9A',lineHeight:1.8,fontStyle:'italic',textAlign:'center'}}>"Páginas escritas pelos escribas do Mestre — registros do que o grupo descobriu, ganhou ou testemunhou ao longo de sua jornada."</div>
      ...
      {pages.map(p=>(
        <div key={p.id} style={{marginBottom:16,border:'1px solid rgba(168,85,247,0.3)',borderRadius:12,background:'rgba(10,12,28,0.95)',overflow:'hidden'}}>
          <button onClick={()=>setOpen(open===p.id?null:p.id)} style={{width:'100%',padding:'14px 18px',display:'flex',alignItems:'center',gap:12,background:open===p.id?'rgba(168,85,247,0.08)':'transparent',border:'none',cursor:'pointer',textAlign:'left'}}>
            <span style={{fontSize:18}}>📜</span>
            <div style={{flex:1}}>
              <div style={{fontFamily:'Cinzel,serif',fontSize:14,color:'#C8A8E8',fontWeight:700}}>{p.titulo||'Página em Branco'}</div>
              <div style={{fontSize:10,color:'#7A6A8A',marginTop:4}}>Descoberto em {p.dataAquisicao}</div>
            </div>
            <span style={{color:'rgba(168,85,247,0.5)',transform:open===p.id?'rotate(90deg)':'none',transition:'transform 0.3s'}}>▶</span>
          </button>
          {open===p.id&&(
            <div style={{padding:'0 18px 18px',borderTop:'1px solid rgba(168,85,247,0.1)'}}>
              <div style={{height:12}}/>
              {masterMode?(
                <div style={{display:'flex',flexDirection:'column',gap:10}}>
                  <input value={p.titulo} onChange={e=>upd(p.id,{...p,titulo:e.target.value})} placeholder="Título do Registro..." style={{fontSize:14,fontFamily:'Cinzel,serif',color:'#E8D8C0'}}/>
                  <textarea value={p.descricao} onChange={e=>upd(p.id,{...p,descricao:e.target.value})} placeholder="Escreva aqui a lenda, o evento, ou a descrição do item raro..." rows={6} style={{fontSize:14,lineHeight:1.8,color:'#9A8A7A',resize:'vertical'}}/>
                  <div style={{display:'flex',justifyContent:'flex-end'}}>
                    <button onClick={()=>del(p.id)} style={{padding:'6px 12px',borderRadius:6,border:'1px solid rgba(232,25,60,0.3)',background:'rgba(232,25,60,0.1)',color:'#E8193C',cursor:'pointer',fontSize:11}}>🗑 Excluir Página</button>
                  </div>
                </div>
              ):(
                <div style={{fontSize:14,lineHeight:1.8,color:'#9A8A7A',whiteSpace:'pre-wrap'}}>{p.descricao||<span style={{fontStyle:'italic',color:'#5A5070'}}>A página está em branco...</span>}</div>
              )}
            </div>
          )}
        </div>
...
// ─── GESTÃO DE ARTEFATOS PELO MESTRE ───────────────────────────────────────────
function ArtefatosMasterSection({ masterMode }) {
  const [unlocked, setUnlocked] = useState({});
  const [habs, setHabs] = useState({});
  const [formHab, setFormHab] = useState({});

  useEffect(() => {
    const u1 = onSnapshot(doc(db, 'config', 'artefatos'), snap => { if (snap.exists()) setUnlocked(snap.data().unlocked || {}); });
    const u2 = onSnapshot(doc(db, 'config', 'artefatos_habilidades'), snap => { if (snap.exists()) setHabs(snap.data() || {}); });
    return () => { u1(); u2(); };
  }, []);

  const toggleReveal = async (id) => {
    const next = { ...unlocked, [id]: !unlocked[id] };
    await setDoc(doc(db, 'config', 'artefatos'), { unlocked: next });
  };

  const saveHab = async (artefatoId) => {
    const novaHab = formHab[artefatoId];
    if (!novaHab || !novaHab.nome) return;
    const atuais = habs[artefatoId] || [];
    const nextHabs = { ...habs, [artefatoId]: [...atuais, { ...novaHab, id: Date.now() }] };
  ...
  const deleteHab = async (artefatoId, habId) => {
    const atuais = habs[artefatoId] || [];
    const nextHabs = { ...habs, [artefatoId]: atuais.filter(h => h.id !== habId) };
    await setDoc(doc(db, 'config', 'artefatos_habilidades'), nextHabs);
  };

  if (!masterMode) return <RestrictedAccess title="O Cofre dos Artefatos" text="Os itens mais poderosos de Cosmum só podem ser forjados e revelados pelo Mestre." />;

  return (
    <div style={{ marginTop: 40, borderTop: '1px solid rgba(232,160,32,0.2)', paddingTop: 40 }}>
      <div style={{ textAlign: 'center', marginBottom: 20 }}>
        <div style={{ fontSize: 11, letterSpacing: '0.4em', color: '#E8A020', fontFamily: 'Cinzel,serif', marginBottom: 10, textTransform: 'uppercase' }}>Forja Cósmica</div>
        <h2 style={{ fontFamily: 'Cinzel Decorative,serif', fontSize: 22, color: '#E8D8C0', fontWeight: 700, margin: 0 }}>Gestão de Artefatos</h2>
        <div style={{ fontSize: 11, color: '#7A6A5A', marginTop: 6, fontFamily: 'Cinzel,serif' }}>Revele artefatos e crie seus poderes para que os jogadores possam equipá-los.</div>
      </div>
      <div style={{ display: 'grid', gap: 16 }}>
        {ARTEFATOS_DATA.map(art => {
          const isRevealed = !!unlocked[art.id];
          const artefatoHabs = habs[art.id] || [];
          const form = formHab[art.id] || { nome: '', descricao: '', custo: '', dano: '', cooldown: '' };
          return (
            <div key={art.id} style={{ border: `1px solid ${isRevealed ? '#E8A020' : 'rgba(255,255,255,0.1)'}`, borderRadius: 12, background: 'rgba(10,12,28,0.95)', padding: 16, transition: 'all 0.3s' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10, flexWrap: 'wrap' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  <span style={{ fontSize: 24, filter: isRevealed ? 'none' : 'grayscale(100%) opacity(0.5)' }}>{art.icon}</span>
                  <div>
                    <div style={{ fontFamily: 'Cinzel,serif', fontSize: 15, color: isRevealed ? '#E8D8C0' : '#7A6A8A', fontWeight: 700 }}>{art.name}</div>
                    <div style={{ fontSize: 11, color: '#5A5070' }}>{artefatoHabs.length} poder(es) registrado(s)</div>
                  </div>
                </div>
                <button onClick={() => toggleReveal(art.id)} style={{ padding: '6px 12px', borderRadius: 8, border: `1px solid ${isRevealed ? 'rgba(232,160,32,0.5)' : 'rgba(255,255,255,0.2)'}`, background: isRevealed ? 'rgba(232,160,32,0.15)' : 'transparent', color: isRevealed ? '#E8A020' : '#7A6A8A', cursor: 'pointer', fontFamily: 'Cinzel,serif', fontSize: 11 }}>
                  {isRevealed ? '👁 Revelado aos Jogadores' : 'Oculto'}
                </button>
              </div>
              {isRevealed && (
                <div style={{ marginTop: 16, paddingTop: 16, borderTop: '1px solid rgba(232,160,32,0.2)' }}>
                  <div style={{ fontSize: 10, letterSpacing: '0.2em', color: '#E8A020', fontFamily: 'Cinzel,serif', marginBottom: 10, textTransform: 'uppercase' }}>Poderes deste Artefato</div>
                  {artefatoHabs.length > 0 && (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 16 }}>
                      {artefatoHabs.map(h => (
                        <div key={h.id} style={{ background: 'rgba(232,160,32,0.05)', border: '1px solid rgba(232,160,32,0.2)', borderRadius: 8, padding: '10px', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                          <div>
                            ...
                            <div style={{ fontSize: 12, color: '#9A8A7A' }}>{h.descricao}</div>
                          </div>
                          <button onClick={() => deleteHab(art.id, h.id)} style={{ background: 'transparent', border: '1px solid rgba(232,25,60,0.3)', color: '#E8193C', borderRadius: 4, padding: '2px 6px', fontSize: 10, cursor: 'pointer' }}>✕</button>
                        </div>
                      ))}
                    </div>
                  )}
                  <div style={{ background: 'rgba(255,255,255,0.02)', padding: 12, borderRadius: 8, border: '1px dashed rgba(232,160,32,0.3)' }}>
                    <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.5)', marginBottom: 8, fontFamily: 'Cinzel,serif' }}>+ Adicionar Novo Poder</div>
                    <input value={form.nome} onChange={e => setFormHab(p => ({ ...p, [art.id]: { ...form, nome: e.target.value } }))} placeholder="Nome do Poder..." style={{ width: '100%', marginBottom: 6, fontSize: 12 }} />
                    <div style={{ display: 'flex', gap: 6, marginBottom: 6 }}>
                      <input value={form.dano} onChange={e => setFormHab(p => ({ ...p, [art.id]: { ...form, dano: e.target.value } }))} placeholder="Dano (ex: 1D12)" style={{ flex: 1, fontSize: 12 }} />
                      <input type="number" value={form.custo} onChange={e => setFormHab(p => ({ ...p, [art.id]: { ...form, custo: e.target.value } }))} placeholder="Custo VC" style={{ width: 80, fontSize: 12 }} />
                      <input value={form.cooldown} onChange={e => setFormHab(p => ({ ...p, [art.id]: { ...form, cooldown: e.target.value } }))} placeholder="Cooldown" style={{ width: 90, fontSize: 12 }} />
                    </div>
                    <textarea value={form.descricao} onChange={e => setFormHab(p => ({ ...p, [art.id]: { ...form, descricao: e.target.value } }))} placeholder="Efeito detalhado..." rows={2} style={{ width: '100%', marginBottom: 6, fontSize: 12, resize: 'vertical' }} />
                    <button onClick={() => saveHab(art.id)} disabled={!form.nome} style={{ width: '100%', padding: '6px', borderRadius: 6, border: '1px solid #E8A020', background: 'rgba(232,160,32,0.1)', color: '#E8A020', cursor: form.nome ? 'pointer' : 'not-allowed', fontFamily: 'Cinzel,serif', fontSize: 11 }}>Salvar Poder</button>
                  </div>
                </div>
              )}
            </div>
...
function LivroMestreSection({ masterMode }) {
  return (
    <div style={{ maxWidth: 760, margin: '0 auto', padding: '40px 24px 80px' }}>
      <div style={{ textAlign: 'center', marginBottom: 32 }}>
        <div style={{ fontSize: 11, letterSpacing: '0.4em', color: '#A855F7', fontFamily: 'Cinzel,serif', marginBottom: 13, textTransform: 'uppercase' }}>Segredos & Relíquias</div>
        <h2 style={{ fontFamily: 'Cinzel Decorative,serif', fontSize: 23, color: '#E8D8C0', fontWeight: 700, margin: 0 }}>Livro do Mestre</h2>
        <div style={{ fontSize: 11, color: '#6A5A7A', marginTop: 7, fontFamily: 'Cinzel,serif' }}>Sincronizado em tempo real</div>
      ...
      <MasterPagesBook masterMode={masterMode} />
      <ArtefatosMasterSection masterMode={masterMode} />
    </div>
  );
}
...
export default function App() {
  ...
  useEffect(() => {
    const u = onSnapshot(doc(db, 'config', 'atmosphere'), snap => {
      if (snap.exists() && snap.data().current) setAtmosphere(snap.data().current);
    });
    return () => u();
  }, []);

  const changeAtmosphere = async (atm) => {
    setAtmosphere(atm);
    await setDoc(doc(db, 'config', 'atmosphere'), { current: atm });
  };

  const handleMasterLogin = () => {
    if (masterPw === MASTER_PIN) { setMasterMode(true); setMasterPw(''); pushToast('Modo Mestre ativado!', '👑', '#A855F7'); }
    else { pushToast('Senha incorreta', '❌', '#E8193C'); setMasterPw(''); }
  };

  return (
    <div style={{ minHeight: '100%', position: 'relative', color: '#C8B8A0', fontFamily: "'Crimson Text', Georgia, serif", display: 'flex', flexDirection: 'column' }}>
      <style>{GLOBAL_CSS}</style>
      <StarField atmosphere={atmosphere} />
      <ToastContainer />
      <AmbientSoundPlayer masterMode={masterMode} />
      {/* O Widget de Atmosfera só renderiza o ícone e está posicionado fixo à esquerda */}
      <AtmosphereWidget masterMode={masterMode} atmosphere={atmosphere} onSet={changeAtmosphere} />
      <DiceWidget />

      <header style={{ position: 'relative', zIndex: 10, padding: '20px 24px', background: 'linear-gradient(to bottom, rgba(4,6,15,0.9), rgba(4,6,15,0))', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        {/* MarginLeft 40 afasta o título do Widget de Atmosfera. A classe header-sub esconde o prefixo no Mobile */}
        <div className="header-title" style={{ fontFamily: "'Cinzel Decorative', serif", fontSize: 24, fontWeight: 900, color: '#E8D8C0', textShadow: '0 0 20px rgba(255,255,255,0.2)', marginLeft: 40 }}>
          <span className="header-sub" style={{ opacity: 0.6, fontWeight: 400, fontSize: '0.8em' }}>Sistema RPG · </span>Dinastia E
        </div>
        <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
          {masterMode ? (
            <button onClick={() => { setMasterMode(false); pushToast('Modo Mestre desativado', '🔒', '#5A5070'); }} style={{ padding: '6px 12px', borderRadius: 8, border: '1px solid rgba(168,85,247,0.4)', background: 'rgba(168,85,247,0.15)', color: '#C8A8E8', fontFamily: 'Cinzel,serif', fontSize: 11, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6 }}><span>👑</span> Mestre Ativo</button>
          ) : (
            <div style={{ display: 'flex', gap: 6 }}>
              <input type="password" value={masterPw} onChange={e => setMasterPw(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleMasterLogin()} placeholder="Senha Mestre" style={{ width: 100, fontSize: 11, padding: '4px 8px' }} />
              <button onClick={handleMasterLogin} style={{ padding: '4px 10px', borderRadius: 6, border: '1px solid rgba(255,255,255,0.1)', background: 'transparent', color: '#9A8A7A', cursor: 'pointer' }}>▶</button>
            </div>
          )}
        </div>
      </header>

      <nav style={{ position: 'relative', zIndex: 10, display: 'flex', justifyContent: 'center', gap: 8, padding: '0 20px', marginBottom: 20 }}>
        {[
          ...
          { id: 'livro', label: 'Livro Mestre' }
        ].map(t => (
          <button key={t.id} onClick={() => setTab(t.id)} style={{ padding: '8px 16px', borderRadius: 20, border: `1px solid ${tab === t.id ? 'rgba(255,255,255,0.3)' : 'transparent'}`, background: tab === t.id ? 'rgba(255,255,255,0.08)' : 'transparent', color: tab === t.id ? '#fff' : 'rgba(255,255,255,0.5)', fontFamily: 'Cinzel,serif', fontSize: 12, letterSpacing: '0.05em', cursor: 'pointer', transition: 'all 0.2s', whiteSpace: 'nowrap' }}>
            {t.label}
          </button>
        ))}
      </nav>
      ...
      <main style={{ flex: 1, position: 'relative', zIndex: 10 }}>
        <div style={{ animation: 'atmosphereShift 0.5s ease' }} key={tab}>
          {tab === 'prologo' && <PrologueSection />}
          {tab === 'regras' && <RulesSection />}
          {tab === 'classes' && <ClassesSection />}
          {tab === 'mapa' && <MapaMundiSection masterMode={masterMode} />}
          ...
          {tab === 'livro' && <LivroMestreSection masterMode={masterMode} />}
        </div>
      </main>
    </div>
  );
