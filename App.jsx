...
function ImageUploadBtn({onImage,label='+ Imagem'}){
  ...
  const handleFile=e=>{const file=e.target.files[0];if(!file)return;const reader=new FileReader();reader.onload=ev=>onImage(ev.target.result);reader.readAsDataURL(file);e.target.value='';};
...
function SheetFull({sheet,onChange,onDelete}){
  ...
  const handlePhotoFile=e=>{const file=e.target.files[0];if(!file)return;const reader=new FileReader();reader.onload=ev=>f('foto',ev.target.result);reader.readAsDataURL(file);};
  return(
    <div style={{border:`1px solid ${sheetColor}44`,borderRadius:16,overflow:'hidden',background:'rgba(8,10,22,0.95)',boxShadow:`0 6px 32px ${sheetGlow}`,animation:'fadeIn 0.4s ease'}}>
      ...
      {/* ── FOTO COMPLETA DO PERSONAGEM ── */}
      <div onClick={()=>photoInputRef.current?.click()} style={{position:'relative',width:'100%',cursor:'pointer',background:'rgba(255,255,255,0.02)',minHeight:sheet.foto?0:120,overflow:'hidden'}}>
        {sheet.foto
          ?<img src={sheet.foto} alt="personagem" style={{width:'100%',maxHeight:340,objectFit:'cover',objectPosition:'center top',display:'block'}}/>
          :<div style={{display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',padding:'30px 20px',gap:8}}>
            <div style={{fontSize:36,opacity:0.2}}>📷</div>
            <div style={{fontSize:11,color:'rgba(255,255,255,0.2)',fontFamily:'Cinzel,serif',letterSpacing:'0.08em'}}>Toque para adicionar foto do personagem</div>
          </div>
        }
        {sheet.foto&&<div style={{position:'absolute',inset:0,background:'linear-gradient(to bottom,transparent 50%,rgba(4,6,15,0.9))',pointerEvents:'none'}}/>}
        {sheet.foto&&(
          <div style={{position:'absolute',bottom:14,left:18,right:18}}>
            <div style={{fontFamily:'Cinzel Decorative,serif',fontSize:20,fontWeight:700,color:sheetColor,textShadow:`0 0 20px ${sheetColor}66`}}>{sheet.nome||'Sem nome'}</div>
            <div style={{fontSize:12,color:'rgba(255,255,255,0.55)',marginTop:3,fontFamily:'Cinzel,serif'}}>{cls.icon} {cls.name} · Nv {sheet.nivel} · {label(sheet.nivel)}</div>
          </div>
        )}
...
function SheetsSection(){
  ...
  const del=async id=>{await deleteDoc(doc(db,'sheets',String(id)));setActiveId(null);};

  const activeSheet=sheets.find(s=>String(s.id)===activeId);

  return(
    <div style={{maxWidth:780,margin:'0 auto',padding:'24px 16px 80px',animation:'fadeIn 0.6s ease'}}>
      <div style={{textAlign:'center',marginBottom:22}}>
        ...
        <div style={{fontSize:11,color:'#4A4050',marginTop:6,fontFamily:'Cinzel,serif'}}>✦ Selecione uma ficha para visualizá-la</div>
      ...
      {loaded&&(
        <>
          {/* ── Tabs por personagem ── */}
          <div className="sheet-tabs-nav" style={{display:'flex',gap:6,marginBottom:20,paddingBottom:2,overflowX:'auto'}}>
            {sheets.map(s=>{
              ...
              return(
                <button key={s.id} onClick={()=>setActiveId(isActive?null:String(s.id))}
                  ...
                  {s.foto
                    ?<img src={s.foto} alt="" style={{width:28,height:28,borderRadius:6,objectFit:'cover',border:`1.5px solid ${sc}44`}}/>
                    :<div style={{width:28,height:28,borderRadius:6,background:`${sc}15`,border:`1.5px dashed ${sc}33`,display:'flex',alignItems:'center',justifyContent:'center',fontSize:14}}>{cls.icon}</div>
                  }
          ...
          {/* ── Ficha ativa ── */}
          {activeSheet
            ?<SheetFull sheet={activeSheet} onChange={d=>upd(activeSheet.id,d)} onDelete={()=>del(activeSheet.id)}/>
...
function EnemyCard({enemy,onChange,onDelete}){
  ...
  const handlePhoto=e=>{const file=e.target.files[0];if(!file)return;const reader=new FileReader();reader.onload=ev=>f('foto',ev.target.result);reader.readAsDataURL(file);};
  return(
    <div style={{border:`1px solid ${ENEMY_COLOR}44`,borderRadius:14,overflow:'hidden',background:'rgba(12,6,6,0.95)',marginBottom:18,boxShadow:`0 4px 24px ${ENEMY_GLOW}`}}>
      ...
      {/* Foto completa do inimigo */}
      <div onClick={()=>photoRef.current?.click()} style={{position:'relative',width:'100%',cursor:'pointer',background:'rgba(255,255,255,0.02)',overflow:'hidden'}}>
        {enemy.foto
          ?<img src={enemy.foto} alt="inimigo" style={{width:'100%',maxHeight:260,objectFit:'cover',objectPosition:'center top',display:'block'}}/>
          :<div style={{display:'flex',alignItems:'center',justifyContent:'center',gap:10,padding:'20px',opacity:0.4}}>
...
function CronicasSection(){
  ...
  const addImage=(entry,img)=>{const updated={...entry,imagens:[...(entry.imagens||[]),img]};upd(entry.id,updated);};
              ...
              <div style={{padding:'0 17px 17px',borderTop:'1px solid rgba(255,255,255,0.05)',animation:'fadeIn 0.3s ease'}}>
                ...
                <div style={{marginBottom:11}}>
                  <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:9}}>
                    ...
                    <ImageUploadBtn onImage={img=>addImage(entry,img)} label="🖼 Adicionar Imagem"/>
                  </div>
                    ...
                    <div style={{display:'flex',flexDirection:'column',gap:10}}>
                      {entry.imagens.map((img,idx)=>(
                        <div key={idx} style={{position:'relative',borderRadius:10,overflow:'hidden',border:'1px solid rgba(255,255,255,0.08)'}}>
                          <img src={img} alt={`Imagem ${idx+1}`} style={{width:'100%',display:'block',maxHeight:400,objectFit:'contain',background:'rgba(0,0,0,0.3)'}}/>
                          <button onClick={()=>removeImage(entry,idx)} style={{position:'absolute',top:8,right:8,background:'rgba(232,25,60,0.85)',border:'none',color:'#fff',borderRadius:6,cursor:'pointer',padding:'4px 9px',fontSize:12,fontFamily:'Cinzel,serif'}}>✕</button>
                        </div>
                      ))}
                    </div>
                  )}
                  {(entry.imagens||[]).length===0&&(
                    <div style={{padding:14,borderRadius:10,border:'1px dashed rgba(255,255,255,0.06)',color:'#5A5070',textAlign:'center',fontSize:11,fontFamily:'Cinzel,serif'}}>Nenhuma imagem adicionada.</div>
                  )}
                </div>
                <div style={{marginTop:4,fontSize:11,color:'#4A4050',textAlign:'right',fontFamily:'Cinzel,serif'}}>{entry.conteudo.length} caracteres · salvo automaticamente</div>
              </div>
            )}
          </div>
        ))}
...
function CenariosSection(){
  ...
  const save=c=>{clearTimeout(saveTimeout.current[c.id]);saveTimeout.current[c.id]=setTimeout(async()=>{await setDoc(doc(db,'cenarios',String(c.id)),c);},700);};
  ...
  const handleMap=(c,e)=>{const file=e.target.files[0];if(!file)return;const reader=new FileReader();reader.onload=ev=>upd(c.id,{...c,mapa:ev.target.result});reader.readAsDataURL(file);e.target.value='';};
        ...
        {cenarios.map(c=>(
          <div key={c.id} style={{border:'1px solid rgba(168,85,247,0.14)',borderRadius:14,marginBottom:14,overflow:'hidden',background:'rgba(8,10,22,0.9)'}}>
            ...
            <div onClick={()=>setOpen(open===c.id?null:c.id)} style={{padding:'14px 18px',display:'flex',alignItems:'center',gap:12,cursor:'pointer',userSelect:'none'}}>
              {c.mapa
                ?<img src={c.mapa} alt="" style={{width:52,height:38,borderRadius:6,objectFit:'cover',flexShrink:0,border:'1px solid rgba(168,85,247,0.25)'}}/>
                :<div style={{width:52,height:38,borderRadius:6,background:'rgba(168,85,247,0.06)',border:'1px dashed rgba(168,85,247,0.2)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:18,flexShrink:0}}>🗺️</div>
              }
              ...
              <div style={{display:'flex',gap:7}}>
                <button onClick={e=>{e.stopPropagation();del(c.id);}} style={{background:'rgba(232,25,60,0.09)',border:'1px solid rgba(232,25,60,0.22)',color:'#E8193C',borderRadius:5,cursor:'pointer',padding:'3px 8px',fontSize:11}}>✕</button>
              ...
              <div style={{borderTop:'1px solid rgba(168,85,247,0.1)',animation:'fadeIn 0.3s ease'}}>
                ...
                {c.mapa&&(
                  <div style={{position:'relative',cursor:'pointer'}} onClick={()=>mapRefs.current[c.id]?.click()}>
                    <img src={c.mapa} alt="mapa" style={{width:'100%',display:'block',maxHeight:480,objectFit:'contain',background:'rgba(0,0,0,0.4)'}}/>
                ...
                <div style={{padding:'16px 18px',display:'flex',flexDirection:'column',gap:13}}>
                  ...
                  <div>
                    <label style={{fontSize:10,letterSpacing:'0.3em',color:'#5A5070',fontFamily:'Cinzel,serif',display:'block',marginBottom:5,textTransform:'uppercase'}}>Nome do Cenário</label>
                    <input value={c.nome} onChange={e=>upd(c.id,{...c,nome:e.target.value})} placeholder="Ex: A Taverna dos Três Mundos, Torre do Esquecimento..." style={{width:'100%'}}/>
                  </div>
                  <div>
                    <label style={{fontSize:10,letterSpacing:'0.3em',color:'#5A5070',fontFamily:'Cinzel,serif',display:'block',marginBottom:5,textTransform:'uppercase'}}>Descrição & Notas</label>
                    <textarea value={c.descricao} onChange={e=>upd(c.id,{...c,descricao:e.target.value})} placeholder={"Descreva este local — sua atmosfera, o que os personagens encontraram aqui, segredos revelados, eventos marcantes..."} rows={5} style={{width:'100%',resize:'vertical',lineHeight:1.85}}/>
                  </div>
