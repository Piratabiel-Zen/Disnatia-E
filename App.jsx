...
@media(max-width:600px){
  ...
  .vigos-dots button span{width:7px!important;height:7px!important;}
  .photo-upload{width:80px!important;height:80px!important;}
  .sheet-photo-attrs{flex-direction:column!important;align-items:stretch!important;}
  .sheet-photo-attrs .photo-upload{width:100%!important;height:72px!important;border-radius:8px!important;}
  ...
  .sheet-header-vigor{display:none!important;}
  .nome-classe-row{flex-direction:column!important;align-items:stretch!important;}
  .nome-classe-row>div,
  .nome-classe-row select{width:100%!important;}
...
const ARTEFATOS_DATA=[
  {id:'artefato-1',name:'O Cristal Cristalizado da Gota de Água',icon:'💎',
   lore:`Ela é um artefato muito poderoso, expelido do corpo do próprio Homem Água. O usuário que o carrega ganha.... (o livro não descreve)`,
   fisico:`Localização: Desconhecida\nOrigem: Corpo do Homem Água`},
  {id:'artefato-2',name:'Sandaliers Six',icon:'👟',
   lore:`Quem possui esse artefato pode estar onde bem entender, espaço e tempo não o param, podendo se movimentar de forma livre em qualquer momento, um teleporte instantâneo.`,
   fisico:`Localização: Desconhecida\nOrigem: Desconhecida`},
  {id:'artefato-3',name:'Artefato III',icon:'◆',lore:'',fisico:''},
...
const RULES_DATA=[
  ...
  {icon:'⚡',title:'Teste de Reflexo',body:`Quando um personagem estiver com apenas 1 Vigor Cósmico restante, ele pode tentar um Teste de Reflexo para desviar de um ataque iminente — mesmo sem gastar VC.\n\nComo funciona:\n• Role 1D20.\n• Se o resultado for 16 ou mais, o personagem esquiva do ataque com sucesso.\n• Se o resultado for 15 ou menos, o ataque acerta normalmente.\n\nLimitação:\n• O Teste de Reflexo só pode ser usado 2 vezes por combate.\n• Não pode ser combinado com a Esquiva normal no mesmo turno.`},
...
function SheetCard({sheet,onChange,onDelete,isOpen,onToggle}){
  ...
  const hpBarPct=Math.min(100,(hp/Math.max(25,hp))*100);
  const hpBonusBarPct=Math.min(100,(hpBonus/Math.max(10,hpBonus))*100);
  return(
    <div style={{border:`1px solid ${isOpen?sheetColor+'66':sheetColor+'28'}`,borderRadius:14,overflow:'hidden',background:'rgba(8,10,22,0.9)',marginBottom:12,boxShadow:isOpen?`0 4px 28px ${sheetGlow}`:'none',transition:'box-shadow 0.3s,border-color 0.3s'}}>
      <div style={{height:3,background:`linear-gradient(90deg,${sheetColor},transparent)`}}/>

      {/* ── HEADER MINIMIZADO (sempre visível) ── */}
      <div onClick={onToggle} style={{padding:'13px 18px',display:'flex',alignItems:'center',gap:13,cursor:'pointer',userSelect:'none'}}>
        {/* Foto miniatura */}
        {sheet.foto
          ?<img src={sheet.foto} alt="" style={{width:42,height:42,borderRadius:8,objectFit:'cover',flexShrink:0,border:`1.5px solid ${sheetColor}44`}}/>
          :<div style={{width:42,height:42,borderRadius:8,background:`${sheetColor}10`,border:`1.5px dashed ${sheetColor}33`,flexShrink:0,display:'flex',alignItems:'center',justifyContent:'center',fontSize:18}}>{cls.icon}</div>
        }
        {/* Info */}
        <div style={{flex:1,minWidth:0}}>
          <div style={{fontFamily:'Cinzel,serif',fontSize:14,fontWeight:700,color:sheetColor,letterSpacing:'0.03em',whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}}>
            {sheet.nome||<span style={{color:'rgba(255,255,255,0.22)',fontStyle:'italic'}}>Sem nome</span>}
          </div>
          <div style={{fontSize:11,color:'#5A5070',marginTop:2,fontFamily:'Cinzel,serif',letterSpacing:'0.04em'}}>
            {cls.icon} {cls.name} · Nv {sheet.nivel||1} · {label(sheet.nivel||1)}
          </div>
        </div>
        {/* HP rápido */}
        <div style={{flexShrink:0,textAlign:'center',padding:'4px 10px',borderRadius:7,background:'rgba(232,25,60,0.08)',border:'1px solid rgba(232,25,60,0.18)'}}>
          <div style={{fontSize:9,color:'rgba(232,25,60,0.6)',fontFamily:'Cinzel,serif',letterSpacing:'0.15em',marginBottom:1}}>HP</div>
          <div style={{fontFamily:'Cinzel,serif',fontSize:15,fontWeight:700,color:'#E8193C',lineHeight:1}}>{hp}</div>
          {hpBonus>0&&<div style={{fontSize:10,color:'#4ADE80',marginTop:1}}>+{hpBonus}</div>}
        </div>
        {/* Vigor dots mini */}
        <div className="sheet-header-vigor" style={{flexShrink:0,display:'flex',gap:4}}>
          {Array.from({length:5}).map((_,i)=>(
            <div key={i} style={{width:9,height:9,borderRadius:'50%',background:i<(sheet.vigos||0)?sheetColor+'88':'rgba(255,255,255,0.08)',border:`1px solid ${i<(sheet.vigos||0)?sheetColor+'66':'rgba(255,255,255,0.1)'}`}}/>
          ))}
        </div>
        {/* Chevron */}
        <div style={{color:sheetColor,fontSize:11,transform:isOpen?'rotate(90deg)':'none',transition:'transform 0.3s',flexShrink:0,opacity:0.7}}>▶</div>
      </div>

      {/* ── CONTEÚDO EXPANDIDO ── */}
      {isOpen&&(
      <div style={{borderTop:`1px solid ${sheetColor}22`,animation:'fadeIn 0.3s ease'}}>
      <div style={{padding:'16px 18px'}}>
        <div className="nome-classe-row" style={{display:'flex',gap:10,alignItems:'flex-end',marginBottom:16,flexWrap:'wrap'}}>
          ...
          <button onClick={e=>{e.stopPropagation();onDelete();}} style={{background:'rgba(232,25,60,0.1)',border:'1px solid rgba(232,25,60,0.3)',color:'#E8193C',borderRadius:6,cursor:'pointer',padding:'6px 11px',fontSize:12}}>✕</button>
        </div>
        ...
        <div className="sheet-stats-grid" style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(138px,1fr))',gap:9,marginBottom:16}}>
          ...
          {/* HP + HP Bonus */}
          <div style={{background:'rgba(232,25,60,0.07)',border:'1px solid rgba(232,25,60,0.2)',borderRadius:10,padding:'12px 14px',gridColumn:'span 1'}}>
            ...
            <div style={{marginTop:10,paddingTop:8,borderTop:'1px solid rgba(255,255,255,0.06)'}}>
              ...
              <div style={{marginTop:5,height:3,background:'rgba(255,255,255,0.06)',borderRadius:2}}>
                <div style={{height:'100%',width:`${hpBonus>0?hpBonusBarPct:0}%`,background:'#4ADE80',borderRadius:2,transition:'width 0.3s'}}/>
              </div>
        ...
        {/* Foto + Atributos */}
        <div className="sheet-photo-attrs" style={{marginBottom:15,display:'flex',gap:16,alignItems:'flex-start'}}>
...
function SheetsSection(){
  ...
  const add=()=>{if(sheets.length>=5)return;const s=newSheet(Date.now());setDoc(doc(db,'sheets',String(s.id)),s);setOpenId(String(s.id));};
  const upd=(id,data)=>{setSheets(prev=>prev.map(s=>s.id===id?data:s));saveSheet(data);};
  const del=async id=>{await deleteDoc(doc(db,'sheets',String(id)));if(openId===String(id))setOpenId(null);};
  ...
  return(
    <div style={{maxWidth:760,margin:'0 auto',padding:'40px 24px 80px',animation:'fadeIn 0.6s ease'}}>
      <div style={{textAlign:'center',marginBottom:28}}>
        ...
        <div style={{fontSize:11,color:'#4A4050',marginTop:7,fontFamily:'Cinzel,serif'}}>✦ Clique em uma ficha para expandi-la · Sincronizado em tempo real</div>
      ...
      {sheets.map(s=><SheetCard key={s.id} sheet={s} onChange={d=>upd(s.id,d)} onDelete={()=>del(s.id)} isOpen={openId===String(s.id)} onToggle={()=>toggle(String(s.id))}/>)}
      {loaded&&sheets.length<5&&(<button onClick={add} onMouseOver={e=>e.currentTarget.style.borderColor='rgba(168,85,247,0.4)'} onMouseOut={e=>e.currentTarget.style.borderColor='rgba(255,255,255,0.11)'} style={{marginTop:6,width:'100%',padding:13,borderRadius:10,border:'1px dashed rgba(255,255,255,0.11)',background:'rgba(255,255,255,0.02)',color:'#7B6D8A',cursor:'pointer',fontFamily:'Cinzel,serif',fontSize:12,letterSpacing:'0.08em',transition:'border-color 0.2s'}}>+ Adicionar Personagem ({sheets.length}/5)</button>)}
    </div>
  );
}
  ...
  return(
    <div style={{background:'rgba(255,255,255,0.02)',border:`1px solid ${color}22`,borderRadius:10,padding:'12px 14px',display:'flex',flexDirection:'column',gap:10}}>
      <div style={{display:'flex',gap:10,alignItems:'flex-end',flexWrap:'wrap'}}>
        <div style={{flex:1,minWidth:180}}><label style={{fontSize:10,letterSpacing:'0.25em',color:'#7A4040',fontFamily:'Cinzel,serif',display:'block',marginBottom:5,textTransform:'uppercase'}}>Nome da habilidade</label><input value={skill.nome} onChange={e=>update('nome',e.target.value)} placeholder="Ex: Golpe Sombrio" style={{width:'100%'}}/></div>
        <div style={{width:120}}><label style={{fontSize:10,letterSpacing:'0.25em',color:'#7A4040',fontFamily:'Cinzel,serif',display:'block',marginBottom:5,textTransform:'uppercase'}}>Dano</label><input value={skill.dano} onChange={e=>update('dano',e.target.value)} placeholder="Ex: 1D8 + 2" style={{width:'100%'}}/></div>
        <div style={{width:100}}><label style={{fontSize:10,letterSpacing:'0.25em',color:'#7A4040',fontFamily:'Cinzel,serif',display:'block',marginBottom:5,textTransform:'uppercase'}}>Custo VC</label><input type="number" min={0} value={skill.cost} onChange={e=>update('cost',Number(e.target.value))} style={{width:'100%'}}/></div>
        <div style={{width:130}}><label style={{fontSize:10,letterSpacing:'0.25em',color:'#7A4040',fontFamily:'Cinzel,serif',display:'block',marginBottom:5,textTransform:'uppercase'}}>Tempo de uso</label><input value={skill.tempo} onChange={e=>update('tempo',e.target.value)} placeholder="Ex: 3 rodadas" style={{width:'100%'}}/></div>
        <div style={{width:120}}><label style={{fontSize:10,letterSpacing:'0.25em',color:'#7A4040',fontFamily:'Cinzel,serif',display:'block',marginBottom:5,textTransform:'uppercase'}}>Tipo</label><select value={skill.tipo} onChange={e=>update('tipo',e.target.value)} style={{width:'100%'}}><option value="normal">Normal</option><option value="especial">Especial</option><option value="passiva">Passiva</option></select></div>
      ...
      <div><label style={{fontSize:10,letterSpacing:'0.25em',color:'#7A4040',fontFamily:'Cinzel,serif',display:'block',marginBottom:5,textTransform:'uppercase'}}>Descrição</label><textarea value={skill.descricao} onChange={e=>update('descricao',e.target.value)} placeholder="Descreva o efeito, alcance, condição ou comportamento desta habilidade..." rows={3} style={{width:'100%',resize:'vertical',lineHeight:1.7}}/></div>
...
function EnemyCard({enemy,onChange,onDelete}){
  ...
  const hp=enemy.hp||0;
  const hpBonus=enemy.hp_bonus||0;
            ...
            {ARTEFATOS_DATA.map((art,i)=>{
              ...
              return(
                <div key={art.id} style={{border:`1px solid ${revealed?'rgba(232,160,32,0.3)':'rgba(255,255,255,0.05)'}`,borderRadius:11,background:revealed?'rgba(232,160,32,0.04)':'rgba(255,255,255,0.014)',overflow:'hidden'}}>
                  ...
                  {revealed&&hasContent&&(
                    <div style={{padding:'14px 16px',display:'flex',flexDirection:'column',gap:12,borderTop:'1px solid rgba(232,160,32,0.1)'}}>
                      <div>
                        <div style={{fontSize:10,letterSpacing:'0.3em',color:'rgba(232,160,32,0.6)',fontFamily:'Cinzel,serif',marginBottom:7,textTransform:'uppercase'}}>Descrição</div>
                        <div style={{fontSize:14,color:'#9A8A7A',lineHeight:1.85,fontStyle:'italic',whiteSpace:'pre-line'}}>{art.lore}</div>
                      </div>
                      {art.fisico&&(<>
                        <div style={{height:1,background:'rgba(232,160,32,0.1)'}}/>
                        <div>
                          <div style={{fontSize:10,letterSpacing:'0.3em',color:'rgba(232,160,32,0.6)',fontFamily:'Cinzel,serif',marginBottom:7,textTransform:'uppercase'}}>Localização & Origem</div>
                          <div style={{fontSize:14,color:'#9A8A7A',lineHeight:1.85,fontStyle:'italic',whiteSpace:'pre-line'}}>{art.fisico}</div>
                        </div>
                      </>)}
                    </div>
                  )}
